...
<template>
  <div class="mb-2">
    <div class="flex gap-2 items-center">
      ...
      >
        <div class="whitespace-nowrap" :class="{ 'font-bold': isVisible, 'pt-1': !isVisible }">
          {{ project.title }}
        </div>
      </div>
    ...
    <div
      ...
      v-show="isVisible"
    >
      <ShortInfo v-show="isVisible" :category="category" :short_info="short_info" :showAll="true" />

      <!-- live -->
      <p class="flex gap-1">
        <span class="label">live:</span>
        <template v-if="project.live.startsWith('http')">
          <a class="underline line-clamp-1" target="_blank" :href="project.live">{{ project.live }}</a>
        </template>
      ...
      <p class="flex gap-1">
        ...
        <template v-if="project.repo !== 'none'">
          <a class="underline line-clamp-1" target="_blank" :href="project.repo">{{ project.repo }}</a>
        </template>
        <template v-else>
